...
{
    /// <summary>
    /// MAssetsのDBAccessorクラスを定義します。
    /// </summary>
    public partial class BSMAssetsAccessor : CommonLogic
    {
        /// <summary>MAssetsのDao</summary>
        protected MAssetsDao Dao = null;
        ...
        /// <summary>DbContext</summary>
        public string TableDbName = "M_ASSETS"; 
        ...
        public BSMAssetsAccessor(ApplicationDbContext context) : base(context)
        {
            Dao = new MAssetsDao(context);
            Context = context;
        }
        #endregion
        ...
        public IList<MAssets> Get()
        {
            return Query<MAssets>("SELECT * FROM M_ASSETS ");
        }
        ...
        /// <returns>件数</returns>
        public Response MAssetsCUD(IList<MAssets> datalist)
        {
        ...
        /// <returns>件数</returns>
        public virtual Response MAssetsCUD(MAssets data)
        {
            Response _Response = new Response()
            {
                Status = false,
        ...
        /// <param name="dataList">更新用データリスト</param>
        /// <returns>件数</returns>
        public Response MAssetsBulkCUD(IList<MAssets> datalist)
        {
            Response _Response = new Response()
            {
                Status = false,
            ...
            // インサート用データ
            IList<MAssets> inserList = new List<MAssets>();
            // アップデート用データ
            IList<MAssets> updateList = new List<MAssets>();
            // デリート用データ
            IList<MAssets> deleteList = new List<MAssets>();
        ...
        public MAssets Find(MAssets data)
        {
            var rec = Dao.Find(data.Id);
            if (rec == null)
            {
                 OriginalException.ExclusiveException("MAssets Key=" + ((long)data.Id).ToString());
            }
        ...
        public int InsetdataList(IList<MAssets> inserList)
        {
            int cnt = 0;
            var sql = string.Empty;
            IList<MAssets> dataList = new List<MAssets>();
        ...
        /// <returns>更新件数</returns>
        public int UpdateList(IList<MAssets> UpdateList)
        {
            int cnt = 0;
            var sql = string.Empty;
            IList<MAssets> dataList = new List<MAssets>();
        ...
        public string GetInsertSql(IList<MAssets> inserList)
        {
        ...
        /// <returns>SQL</returns>
        public string EditInsertSql(MAssets entity)
        {
            var sql = new StringBuilder();
            sql.Append(" " + GenericUtil.LongToString(entity.Id) + ", ");
            sql.Append(" " + GenericUtil.LongToString(entity.AssetsId) + ", ");
            sql.Append(" " + GenericUtil.StringToString(entity.SuitNo) + ", ");
            sql.Append(" " + GenericUtil.StringToString(entity.AssetType) + ", ");
            sql.Append(" " + GenericUtil.LongToString(entity.Kingaku) + ", ");
            sql.Append(" " + GenericUtil.DateToString(entity.DueDate) + ", ");
            sql.Append(" " + GenericUtil.StringToString(entity.PaymentMethod) + ", ");
            sql.Append(" " + GenericUtil.StringToString(entity.TransactionDetails) + ", ");
            sql.Append(" " + GenericUtil.StringToString(entity.Payer) + ", ");
        ...
        /// <returns>SQL文</returns>
        public string GetUpdateSql(IList<MAssets> updateList)
        {
            var now = DateTime.Now;
            var strSql = string.Empty;
        ...
        /// <param name="entity">エンティティ</param>
        /// <returns>SQL</returns>
        public string EditUpdateSql(MAssets entity)
        {
            var sql = new StringBuilder();
            sql.Append("UPDATE M_ASSETS");
            sql.Append(" SET");
            sql.Append("   ASSETS_ID  = " + GenericUtil.LongToString(entity.AssetsId));
            sql.Append(",   SUIT_NO  = " + GenericUtil.StringToString(entity.SuitNo));
            sql.Append(",   ASSET_TYPE  = " + GenericUtil.StringToString(entity.AssetType));
            sql.Append(",   KINGAKU  = " + GenericUtil.LongToString(entity.Kingaku));
            sql.Append(",   DUE_DATE  = " + GenericUtil.DateToString(entity.DueDate));
            sql.Append(",   PAYMENT_METHOD  = " + GenericUtil.StringToString(entity.PaymentMethod));
            sql.Append(",   TRANSACTION_DETAILS  = " + GenericUtil.StringToString(entity.TransactionDetails));
            sql.Append(",   PAYER  = " + GenericUtil.StringToString(entity.Payer));
        ...
        /// <returns>SQL</returns>
        public string CreateCSV(IList<MAssets> dataList, bool header = true)
        {
            var csv = new StringBuilder();
            if (header)
            {
                csv.Append("ID" + "\t");
                csv.Append("ASSETS_ID" + "\t");
                csv.Append("SUIT_NO" + "\t");
                csv.Append("ASSET_TYPE" + "\t");
                csv.Append("KINGAKU" + "\t");
                csv.Append("DUE_DATE" + "\t");
                csv.Append("PAYMENT_METHOD" + "\t");
                csv.Append("TRANSACTION_DETAILS" + "\t");
                csv.Append("PAYER" + "\t");
            ...
            foreach (var data in dataList)
            {
                ...
                csv.Append(data.AssetsId.ToString() + "\t");
                csv.Append(data.SuitNo + "\t");
                csv.Append(data.AssetType + "\t");
                csv.Append(data.Kingaku.ToString() + "\t");
                csv.Append(((DateTime)data.DueDate).ToString("yyyy/MM/dd") + "\t");
                csv.Append(data.PaymentMethod + "\t");
                csv.Append(data.TransactionDetails + "\t");
                csv.Append(data.Payer + "\t");
        ...
        public virtual void DefaultSet(MAssets data)
        {
        }
