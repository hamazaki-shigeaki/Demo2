...
namespace DBAccess.Dao
{
    ...
    /// TOcrのDaoクラスを定義します。
    /// </summary>
    public partial class TOcrDao : CommonLogic
    {
        /// <summary>DBコンテキスト</summary>
        private ApplicationDbContext con = null;

        #region "コンストラクタ"
        public TOcrDao(ApplicationDbContext context) : base(context)
        {
            con = context;
        }
        #endregion
        ...
        /// </summary>
        /// <param name="entity">エンティティ</param>
        public int Delete(TOcr entity)
        {
            var sql = new StringBuilder(); 
            sql.Append("DELETE FROM T_OCR");
            sql.Append($" WHERE id = @Id and update_datetime = @UpdateDatetime ");
            var param = new DynamicParameters();
            param.Add("Id", entity.Id);
            param.Add("UpdateDatetime", entity.UpdateDatetime);
        ...
        public TOcr Find(long? id)
        {
            var cb = new TOcrCB(); 
            cb.Query().SetId_Equal(id); 
            return ExecuteEntity<TOcr>(cb); 
        }
        #endregion
